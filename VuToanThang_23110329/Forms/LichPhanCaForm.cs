...
namespace VuToanThang_23110329.Forms
{
    ...
    public partial class TaoLichTuanForm : Form
    {
        ...
        private ComboBox cmbNhanVien, cmbCaLam;
        private DateTimePicker dtpTuNgay;
        private CheckBox[] chkNgayTuan;
        private Button btnTaoLich, btnHuy;

        public TaoLichTuanForm()
        {
            _lichPhanCaRepository = new LichPhanCaRepository();
            _nhanVienRepository = new NhanVienRepository();
            _caLamRepository = new CaLamRepository();
        ...
        private void InitializeComponent()
        {
            ...
            // Tạo checkbox cho các ngày trong tuần
            chkNgayTuan = new CheckBox[7];
            string[] tenNgay = { "Thứ 2", "Thứ 3", "Thứ 4", "Thứ 5", "Thứ 6", "Thứ 7", "Chủ nhật" };
            
            for (int i = 0; i < 7; i++)
            {
                chkNgayTuan[i] = new CheckBox
                {
                    Text = tenNgay[i],
                    ForeColor = Color.White,
                    Location = new Point(20 + (i % 4) * 100, 220 + (i / 4) * 30),
                    AutoSize = true,
                    Checked = i < 5 // Mặc định chọn thứ 2-6
                };
            }
            ...
            var controls = new List<Control> { lblTitle, lblNhanVien, cmbNhanVien, lblCaLam, cmbCaLam, lblTuNgay, dtpTuNgay, lblNgayTuan, btnTaoLich, btnHuy };
            controls.AddRange(chkNgayTuan);
        ...
        private void LoadData()
        {
            ...
            {
                // Load nhân viên
                var nhanViens = _nhanVienRepository.GetAll().Where(nv => nv.TrangThai == "DangLam").ToList();
                cmbNhanVien.DataSource = nhanViens;
                cmbNhanVien.DisplayMember = "HoTen";
                cmbNhanVien.ValueMember = "MaNV";

                // Load ca làm
                var caLams = _caLamRepository.GetAll().Where(ca => ca.KichHoat).ToList();
                cmbCaLam.DataSource = caLams;
                cmbCaLam.DisplayMember = "TenCa";
                cmbCaLam.ValueMember = "MaCa";
            }
        ...
        private void btnTaoLich_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbNhanVien.SelectedValue == null || cmbCaLam.SelectedValue == null)
                {
                    MessageBox.Show("Vui lòng chọn nhân viên và ca làm!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                var selectedDays = chkNgayTuan.Where((cb, index) => cb.Checked).Select((cb, index) => index).ToList();
                
                if (selectedDays.Count == 0)
                {
                    MessageBox.Show("Vui lòng chọn ít nhất một ngày trong tuần!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                int maNV = (int)cmbNhanVien.SelectedValue;
                int maCa = (int)cmbCaLam.SelectedValue;
                DateTime tuNgay = dtpTuNgay.Value.Date;

                // Tìm thứ 2 của tuần
                DateTime startOfWeek = tuNgay.AddDays(-(int)tuNgay.DayOfWeek + 1);
                if (tuNgay.DayOfWeek == DayOfWeek.Sunday)
                    startOfWeek = startOfWeek.AddDays(-7);

                var danhSachLich = new List<LichPhanCa>();

                foreach (int dayIndex in selectedDays)
                {
                    DateTime ngayLam = startOfWeek.AddDays(dayIndex);
                    
                    danhSachLich.Add(new LichPhanCa
                    {
                        MaNV = maNV,
                        MaCa = maCa,
                        NgayLam = ngayLam,
                        TrangThai = "Mo",
                        GhiChu = "Tạo lịch tuần tự động"
                    });
                }

                var result = _lichPhanCaRepository.BulkInsert(danhSachLich);
            ...
            {
                MessageBox.Show($"Lỗi tạo lịch tuần: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
